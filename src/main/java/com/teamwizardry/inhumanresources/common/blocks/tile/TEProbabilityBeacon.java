		...
		if (world.getTotalWorldTime() % 20L == 0L)
		{
			...
			if (structureError != null)
			{
				...
				if (player != null && world.isRemote)
					player.sendMessage(new TextComponentString(structureError));
			}
			...
			else if (!isFormed && active)
				world.setBlockState(pos, state.withProperty(BlockProbabilityBeacon.ACTIVE, false), 2);
		}
